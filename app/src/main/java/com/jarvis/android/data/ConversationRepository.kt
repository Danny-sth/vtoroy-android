...
import com.jarvis.android.network.ConversationResponse
import com.jarvis.android.network.JarvisApiClient
import com.jarvis.android.network.MessageResponse
...
class ConversationRepository @Inject constructor(
    ...
    /**
     * Refresh messages from API (pull-on-focus strategy)
     */
    suspend fun refreshMessages(authToken: String, conversationId: String) {
        try {
            ...
            if (apiResult.isSuccess) {
                val messages = apiResult.getOrNull() ?: emptyList()
                messageDao.insertMessages(messages.map { it.toEntity() })
                Log.d(TAG, "✅ Refreshed ${messages.size} messages")
            } else {
                Log.e(TAG, "Failed to refresh messages")
            }
        } catch (e: Exception) {
