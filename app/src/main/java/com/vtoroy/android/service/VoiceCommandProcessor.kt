...
class VoiceCommandProcessor @Inject constructor(
    ...
    interface StateCallback {
        fun onStateChanged(state: VtoroyState)
    ...
    suspend fun processVoiceCommand(callback: StateCallback): ProcessingResult {
        return try {
            ...
            callback.onStateChanged(VtoroyState.LISTENING)
            ...
            callback.onStateChanged(VtoroyState.PROCESSING)
            ...
            val processingResult = when (result) {
                is VtoroyApiClient.ApiResult.Success -> {
                    Log.d(TAG, "Received response: ${result.audioData.size} bytes")
                    callback.onStateChanged(VtoroyState.PLAYING)
                    audioPlayer.playAudio(result.audioData)
                    ProcessingResult.Success
                }
                is VtoroyApiClient.ApiResult.Error -> {
