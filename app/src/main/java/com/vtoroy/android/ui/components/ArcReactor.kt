package com.vtoroy.android.ui.components
...
fun ArcReactor(
    ...
    val (coreColor, glowColor, dimColor) = when (state) {
        VtoroyState.IDLE -> Triple(ArcReactorColors.idleCore, ArcReactorColors.idleGlow, ArcReactorColors.idleDim)
        VtoroyState.LISTENING, VtoroyState.RECORDING -> Triple(ArcReactorColors.listeningCore, ArcReactorColors.listeningGlow, ArcReactorColors.listeningDim)
        VtoroyState.PROCESSING -> Triple(ArcReactorColors.processingCore, ArcReactorColors.processingGlow, ArcReactorColors.processingDim)
        VtoroyState.PLAYING -> Triple(ArcReactorColors.playingCore, ArcReactorColors.playingGlow, ArcReactorColors.playingDim)
        VtoroyState.ERROR -> Triple(ArcReactorColors.errorCore, ArcReactorColors.errorGlow, ArcReactorColors.errorDim)
    ...
    val pulseSpeed = when (state) {
        VtoroyState.LISTENING, VtoroyState.RECORDING -> 400
        VtoroyState.PROCESSING -> 200
        VtoroyState.PLAYING -> 600
        VtoroyState.ERROR -> 150
        else -> 2000
    }
    ...
    val rotationAngle by infiniteTransition.animateFloat(
        ...
        animationSpec = infiniteRepeatable(
            animation = tween(
                durationMillis = if (state == VtoroyState.PROCESSING) 1000 else 3000,
                easing = LinearEasing
            ),
    ...
    val scale by infiniteTransition.animateFloat(
        ...
        targetValue = when (state) {
            VtoroyState.LISTENING, VtoroyState.RECORDING -> 1.08f
            VtoroyState.PLAYING -> 1.05f
            else -> 1f
        },
        animationSpec = infiniteRepeatable(
            animation = tween(pulseSpeed, easing = FastOutSlowInEasing),
    ...
    val isActive = state != null && state != VtoroyState.ERROR
    val isProcessing = state == VtoroyState.PROCESSING
